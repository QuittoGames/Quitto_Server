...
logger = logging.getLogger("server.repository.system_info")
...
class UserRepository:
    ...
    def close(self):
        ...
        try:
            if self.db:
                self.db.close_all()
                logger.info("[INFO] Conexão com banco de dados fechada")
        except Exception as e:
            logger.error(f"[ERROR] Erro ao fechar conexão: {e}")
